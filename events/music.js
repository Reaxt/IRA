...
music.on("play", (message) =>{
  ...
  if(global.queue.length === 0) {
    message.channel.send({embeds:[utils.embed("happy", `OK I'M DONE BYE`)]})
  ...
  if (global.queue[0].type == "youtube") {
    ...
    message.channel.send({embeds:[utils.embed("track_played", `NOW PLAYING \`${global.queue[0]["info"]}\` QUEUED BY \`${global.queue[0]["user"].username}\` WITH A LENGTH OF \`${global.queue[0]["minutes"]}:${global.queue[0]["seconds"]}\` `, undefined, `https://youtu.be/${footer}`)]})
    ...
    message.channel.send({embeds:[utils.embed("track_played", `NOW PLAYING \`${global.queue[0]["info"]}\` QUEUED BY \`${global.queue[0]["user"].username}\` WITH A LENGTH OF \`${global.queue[0]["minutes"]}:${global.queue[0]["seconds"]}\` `, undefined, global.queue[0].permalink_url)]})
    dispatcher = message.guild.voice.connection.play(request(global.queue[0].url+"?client_id="+config.scid, (error, response) => {
      ...
      message.channel.send({embeds:[utils.embed("track_played", `NOW PLAYING [${global.queue[0]["info"]}](${global.queue[0].url}) QUEUED BY \`${global.queue[0]["user"].username}\``, undefined, undefined)]})
  ...
  dispatcher.on("error", info => {
    console.log({embeds:[utils.embed("malfunction", `OH THAT'S NOT GOOD \`\`\`${info}\`\`\``,"Red")]});
    global.playing = false
  })
