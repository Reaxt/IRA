...
module.exports = {
	...
	fetchRoles:function(message, member, callback) {
		db.findOne({id:member.user.id}, function(err, doc) {
			if (!doc) return message.channel.send({embeds:[utils.embed(`malfunction`,`I DON'T KNOW ANYONE NAMED ${member.user.username}`, "Red")]})
			if (!doc.roles) return message.channel.send({embeds:[utils.embed(`malfunction`,`THERE ISN'T ANYTHING IN MY DATABANKS FOR ${member.user.username}.`, "Red")]})
	...
	buyItem:function(message, user, price, callback) {
		db.findOne({id:user.id}, function(err, doc) {
			...
			if (err) message.channel.send({embeds:[utils.embed(`malfunction`,`OH THAT'S NOT GOOD \`\`\`${err}\`\`\``, "Red")]})
				...
				if (price > doc.coins) {
					message.channel.send({embeds:[utils.embed('angry', "YOU'RE TOO BROKE, GO GET A LOAN")]})
				} else {
					doc.coins -= price
