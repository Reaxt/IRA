...
module.exports = {
  ...
  func:function(message){
    ...
    if(message.member.voice.channelID != message.guild.me.voice.channelID) return message.channel.send({embeds:[utils.embed("sad", "I CAN'T HEAR YOU OVER THERE COME CLOSER")]})
    ...
    if(!global.voteusers.includes(message.author.id)) {
      ...
      message.channel.send({embeds:[utils.embed("happy", `YOU VOTED \`${global.votes}/${needed}\``)]})
    } else {
      message.channel.send({embeds:[utils.embed("sad", `YOU CAN'T VOTE AGAIN \`${global.votes}/${needed}"`)]})
    }
    if(global.votes >= needed) {

      message.channel.send({embeds:[utils.embed("happy",`\`${global.votes}/${needed}\` VOTES RECIEVED NEXT TRACK`)]})
