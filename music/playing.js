...
module.exports = {
  ...
  func:function(message, music){
    ...
    var seconds = ((message.guild.voiceConnection.dispatcher.time % 60000) / 1000).toFixed(0)
    let played = utils.tomins(seconds)

    let footer = ytID(global.queue[0]["url"])
    if(footer === null) footer = global.queue[0]["url"]
    message.channel.send({embed:utils.embed("happy", `\`Playing ${global.queue[0]["info"]}\` \`${played[0]}:${played[1]}/${global.queue[0].minutes}:${global.queue[0].seconds}\``, undefined, `https://youtu.be/${footer}`)})
  }
}
