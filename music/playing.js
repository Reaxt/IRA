...
module.exports = {
  ...
  func:function(message, music){
    ...
    if(!message.guild.voice?.connection?.dispatcher) return message.channel.send({embed:utils.embed("angry", "Im not playing stuff")})
    ...
    if (element.type == "youtube") {
      ...
      let played = utils.tomins(seconds)
      let second = played[1]
      message.channel.send({embed:utils.embed("happy", `Playing [${element["info"]}](${element["url"]}) \`${played[0]}:${second}\` queued by \`${element["user"].username}\``, undefined, undefined)})
    }
  }
}
